...
static void bench_bellman_ford_reversed(benchmark::State &state)
{
    ...
    while (state.KeepRunning()) {
        std::vector<vertex_index_t> path;
        benchmark::DoNotOptimize(simple_graph::bellman_ford(g, state.range(0) - 1, 0, &path));
    }
}
...
static void bench_bellman_ford_random(benchmark::State &state)
{
    ...
    while (state.KeepRunning()) {
        std::vector<vertex_index_t> path;
        benchmark::DoNotOptimize(simple_graph::bellman_ford(g, vertices[0], vertices[state.range(0) - 1], &path));
    }
}
